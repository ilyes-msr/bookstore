    ...
    public function addToCart(Request $request)
    {
        ...
        if (auth()->user()->booksInCart->contains($book)) {
            ...
            if ($newQuantity > $book->number_of_copies) {
                session('warning_message', 'لم تتم إضافة الكتاب، لقد تجاوزت عدد النسخ الموجود لدينا، أقصى عدد يمكنك إضافته هو' . ($book->number_of_copies - auth()->user()->booksInCart()->where('book_id', $book->id)->first()->pivot->number_of_copies) . ' كتاب');
